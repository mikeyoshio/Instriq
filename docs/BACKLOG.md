...
* Nivell 3 restant: **la unificació dels 6 sinònims de rol de permisos** s'ha resolt parcialment (2026-08-11) — afegit `WorkspaceRole.isAdministrator`, substituint l'única comparació ad-hoc (`workspace_detail_screen.dart`) que reinventava aquest concepte sense nom propi; la resta (`ProfileService.isAdmin`, `WorkspaceMember.isHospitalAdmin`, `.effectiveRole`, `.canApprove`, `canApproveAnyWorkspace`) es van revisar i confirmar **legítimament separats**, no redundants (cada un respon una pregunta diferent, ja documentada al codi). **Encara sense abordar**: accessibilitat exhaustiva camp a camp i adopció completa del breakpoint responsive a totes les pantalles — són passades exhaustives sobre les 61 pantalles, mida pròpia, no incloses en aquest tram.
  ...
  **Primer tram fet (2026-09-14)**: auditoria d'accessibilitat/responsive amb troballes concretes abans de tocar res (cap regressió de tooltips des de l'agost; 0 usos de `Semantics(` a tot el codi — terreny verge, sense patró previ; cap pantalla, ni tan sols `catalog_screen.dart`, limitava l'amplada del contingut en escriptori). Nou `InstriqBreakpoints` (`lib/design_system/tokens.dart`, `tablet`/`maxContentWidth`) i `InstriqResponsiveContent` (`lib/design_system/components/`, envoltori "només contingut" — `Center`+`ConstrainedBox`, no-op en mòbil per construcció, mateix criteri que `InstriqVersionDiff`/`InstriqVersionHistory`), substituint la constant `_tabletBreakpoint = 840.0` que `app_shell.dart`/`catalog_screen.dart` duplicaven cadascun pel seu compte. Aplicat a 19 pantalles (les de més trànsit: `home_screen.dart`, 6 `*_detail_screen.dart`, 6 `*_form_screen.dart`, `review_inbox_screen.dart`, gestió d'espai/equips/organització, auditoria, cobertura de coneixement). 4 zones tàctils ampliades amb `Padding` (enllaços de foto/IFU/fabricant/cirurgià que eren `Text` nu dins `GestureDetector`/`InkWell`, per sota del mínim recomanat ~44px). `flutter analyze`/`flutter test` nets. **Fora d'abast d'aquest tram**: la resta de les ~61 pantalles (la majoria, ja de per si més senzilles: llistes/formularis curts), l'ordre de focus per a lector de pantalla, i qualsevol `Semantics` personalitzat (sense necessitat detectada encara — cap control pintat a mida trobat, només widgets Material estàndard amb semàntica per defecte raonable).
...
* ~~Dissenyar el sistema de versionat per a l'instrumental personalitzat.~~ **Fet.** Implementat sobre `supabase/schema_v39_custom_instrument_versioning.sql`, seguint la recepta d'ADR-004 §5: `custom_instruments` passa a ser només capçalera (`organization_id`/`workspace_id`/`published_version_id`), amb `custom_instrument_versions` (draft → in_review → published → archived, `UNIQUE(custom_instrument_id, version_number)` + índex únic parcial "només 1 publicada" des del primer dia). Decisions preses explícitament amb el propietari (no per omissió, tal com exigeix la recepta): (1) les variants (nom/foto/nota) passen de la seva pròpia taula (`custom_instrument_variants`, eliminada) a viure com a jsonb dins de la versió — mateix criteri que `items`/`photo_paths` a `tray_versions` —, migrant les files existents a la primera versió publicada; (2) notificació push en aprovar/rebutjar (`supabase/functions/send-push`, generalitzant `resolveNotificationPlan` en comptes de duplicar-lo per a una 4a entitat — de pas, corregit un bug real trobat pel camí: la funció encara llegia `record.hospital_id`/`profiles.hospital_id`, noms de columna que ja no existeixen des del rename a `organization_id`, schema_v20); (3) sincronitzar `knowledge_links` **no aplica** en aquest sentit — un instrument personalitzat és el `to_id` referenciat per tècniques/safates, no un `from_type` amb relacions pròpies, i el seu id de capçalera no canvia en versionar, així que els enllaços existents ja funcionen sense cap canvi aquí (no és una omissió); (4) esborrat amb RPC pròpia i auditoria (`delete_custom_instrument`, calcada de `delete_group_document`). Codi Dart: `CustomInstrument`/`CustomInstrumentVersion`/`CustomInstrumentVariant` reescrits (el primer exposa getters de conveniència cap a `publishedVersion` per no haver de tocar les ~20 pantalles de només lectura que ja feien `instrument.name`/`.category`/etc.), `CustomInstrumentService` calcat de `TrayService` (sense passar per `SyncQueueService`/`OfflineCacheService` — el mode sense connexió no es va demanar per a aquesta instància, i afegir-lo sense necessitat real seria sobre-enginyeria), formulari i fitxa de detall amb el mateix patró esborrany/enviar a revisió que la resta, historial i diff de versions (reutilitzant `InstriqVersionHistory`/`InstriqVersionDiff` ja genèrics), i 6a pestanya a `ReviewQueueScreen`. **Decisió conscient**: NO es construeix encara el `VersionedContentService` compartit que apunta ADR-004 §3 — amb un únic consumidor nou (aquesta instància), forçar l'abstracció ara seria exactament el tipus de generalització prematura que el mateix ADR-004 descarta per al costat SQL; es deixa documentat per quan hi hagi una 5a instància que en validi la forma real. `flutter analyze`/`flutter test` nets. **Pendent, com sempre**: aplicar la migració al projecte Supabase real (SQL Editor) i provar el flux complet en viu amb compte autenticat (crear → desar esborrany → enviar a revisió → aprovar com a approver → contingut publicat visible).
...
* ~~Facilitar donar d'alta tècniques/protocols que ja existeixen en paper (OCR amb la càmera).~~ **Fet, Android/iOS i Web.** `GroupDocumentFormScreen` guanya "Escaneja un document": fotografia (o, a Web, adjunta) un full imprès i cada línia de text reconeguda s'afegeix com un pas nou editable (mai substitueix els que ja hi hagués; el títol només es rellena si estava buit). **Decisions preses**: només tècniques/protocols (no bandejes/targetes, que no tenen text lliure real a escanejar); no es guarda la foto original, només el text; una línia reconeguda = un pas candidat, l'usuari fusiona/divideix/reordena després amb l'editor de passos ja existent.

  Reconeixement **100% al dispositiu/navegador, mai a un tercer**, amb implementació condicional per plataforma (`lib/services/ocr_service.dart` com a façana + `ocr_recognizer_io.dart`/`ocr_recognizer_web.dart`/`ocr_recognizer_stub.dart`, import condicional `dart.library.io`/`dart.library.js_interop`): Android/iOS via `google_mlkit_text_recognition` (script llatí, model integrat al SDK, sense descàrrega ni xarxa); Web via **Tesseract.js autoallotjat** a `web/tesseract/` (~25 MB, `tesseract.js@7.0.0`/`tesseract.js-core@7.0.0` + `.traineddata` de cat/spa/eng, descarregat amb `Invoke-WebRequest` — mai des d'un CDN de tercers en temps real; recortat de 48 MB originals eliminant les variants WASM que el codi no arriba a demanar mai, ja que només s'usa el mode LSTM), interop modern `dart:js_interop`/`package:web`, càrrega peresosa només en prémer "escanejar", idioma triat segons el locale actual de l'app (ca/es/en → cat/spa/eng). Coherent amb ADR-002/ADR-003: res que depengui de connexió ni que enviï contingut clínic a un tercer, en cap de les dues plataformes.

  Requereix pujar `IPHONEOS_DEPLOYMENT_TARGET` de 12.0 a 15.5 (mínim que exigeix ML Kit) a `ios/Runner.xcodeproj`. **2 bugs preexistents trobats i corregits pel camí**: (1) `ios/Runner/Info.plist` no tenia `NSCameraUsageDescription`/`NSPhotoLibraryUsageDescription` — iOS ho exigeix per a qualsevol ús de càmera/galeria via `image_picker`, així que les fotos de safates/instrumental personalitzat ja estaven trencades a iOS (mai detectat, iOS no s'havia arribat a provar en viu). (2) `TrayService.uploadPhoto`/`CustomInstrumentService.uploadVariantPhoto` embolicaven `image_picker`'s `XFile` en un `dart:io.File` abans de pujar-lo — a Flutter Web, `XFile.path` és una blob: URL, no una ruta de disc real, així que la pujada de fotos ja estava trencada a Web des de sempre. **Corregit**: ambdós mètodes reben ara un `XFile` i pugen els bytes amb `uploadBinary` de Supabase Storage (funciona igual a totes les plataformes, sense necessitat de codi condicional com sí que calia per a l'OCR); la previsualització de foto de variant a `custom_instrument_form_screen.dart` passa d'`Image.file` a `Image.memory` pel mateix motiu.

  Verificat de veritat en Chrome dins d'aquesta mateixa sessió (no només anàlisi estàtica) amb una imatge de prova generada i text reconegut exacte, dues vegades (implementació inicial i després de retallar els assets). `flutter analyze`/`flutter test` nets. **Pendent, no verificable en aquesta sessió**: el camí Android/iOS d'ML Kit i la captura real de càmera des d'un navegador mòbil real no s'han pogut provar — l'entorn és Windows sense SDK d'Android ni Xcode. Provar en un dispositiu real (foto amb poca llum, lletra manuscrita vs. impresa, fulls arrugats...) queda pendent.

* ~~Autoguardat de borradors (tècniques/protocols, safates, targetes de preferència, instrumental personalitzat).~~ **Fet.** Sorgeix d'una revisió UX externa d'aquesta sessió (contrastada punt per punt contra el codi real abans d'actuar-hi — moltes de les seves "incògnites" ja estaven resoltes, però l'absència d'autoguardat era un forat real): fins ara els 4 formularis principals només desaven en prémer "Guardar"/"Enviar a revisió", amb risc real de perdre feina en ús mòbil interromput. Nou `lib/design_system/components/instriq_autosave.dart` (`AutosaveController` amb debounce de 2s + `InstriqAutosaveIndicator`, text discret a l'AppBar), deliberadament compartit entre els 4 formularis des del primer dia — a diferència del patró capçalera+versions d'ADR-004 §3, aquí és pura infraestructura de temporització sense res específic del domini, i els 4 consumidors ja existien simultàniament, no un hipotètic 4t futur. Cada formulari reutilitza el mateix `saveDraft()` del guardat manual (ja sabia encolar-se sense connexió). Fora d'abast deliberat a tots 4: pujar fotos noves i guardar etiquetes, que segueixen només al guardat manual explícit. A la targeta de preferència, a més, l'autoguardat mai resol ni crea un cirurgià nou (`SurgeonService.createOrGet`) només perquè l'usuari hagi escrit mig nom — deixa `surgeonId` tal com estigués ja persistit, només el guardat manual el resol. `flutter analyze`/`flutter test` nets. **Pendent, com sempre**: provar el flux complet en viu amb compte autenticat real (aquest sandbox no en té un).

* ~~Generalitzar "duplicar" (fins ara només bandejes) a tècniques/protocols, targetes de preferència i instrumental personalitzat.~~ **Fet.** Segon punt de la mateixa revisió UX externa: `duplicate_tray` (schema_v25) ja existia però era l'única de les 4 entitats versionades amb aquesta capacitat. Nou `supabase/schema_v40_duplicate_content.sql`: `duplicate_group_document`/`duplicate_preference_card`/`duplicate_custom_instrument`, calcades exactament del patró de `duplicate_tray` (crea una capçalera nova + versió 1 en esborrany amb el contingut de la versió PUBLICADA d'origen, mai d'un esborrany aliè en curs; auditoria; exigeix rol editor/approver/administrator). Deliberadament NO es copien: fotos (cap variant duplicada conserva `photo_path` — mateix criteri que `duplicate_tray` amb `photo_paths`) ni `validated_by_surgeon` (una targeta duplicada és una còpia nova sense validar). Botó "Duplica" (mateixa icona/tooltip que `TrayDetailScreen`) a `group_document_detail_screen.dart`/`preference_card_detail_screen.dart`/`custom_instrument_detail_screen.dart`, visible només quan `canEdit` i hi ha una versió publicada d'origen.

  **Hallazgo real trobat pel camí, corregit de pas**: les 6 RPC noves de `schema_v39_custom_instrument_versioning.sql` (creació/enviament a revisió/aprovar/rebutjar/restaurar/esborrar d'instrumental personalitzat) es van quedar sense el reforç `revoke ... from public / grant ... to authenticated` que `schema_v34_rpc_grants_hardening.sql` ja havia aplicat a totes les altres RPC equivalents — és a dir, `anon` hauria pogut cridar-les (encara que cadascuna es protegeix igualment per dins amb `auth.uid()`/comprovació de rol, mateix risc pràctic baix que documentava schema_v34). Tancat a `schema_v40_duplicate_content.sql`, mateix criteri.
...
* ~~Invitació per email a una persona concreta, amb rol de espai ja assignat d'entrada.~~ **Fet.** Tercer i últim punt de la revisió UX externa d'aquesta sessió. Decisions preses explícitament amb el propietari abans de construir-ho: (1) el codi únic d'organització **es manté** per a l'alta autoservei, la invitació és una via addicional, no un reemplaçament; (2) només Owner/Administrator D'ORGANITZACIÓ pot enviar invitacions (no Administrator d'espai).
  ...
  **Decisió conscient, fora d'abast**: fer algú Administrator d'organització segueix sent una acció separada i posterior (`set_hospital_admin`), no es folda dins la invitació — el rol assignable per invitació és només reader/editor/approver.
  ...
  `flutter analyze`/`flutter test` nets. **Actualització (2026-09-15)**: migració aplicada al projecte Supabase real (via CLI, `supabase db query --linked`), Edge Function desplegada, Database Webhook creat i secret `RESEND_API_KEY` configurat — de pas es va trobar i reparar un forat real de l'aprovisionament del projecte: l'esquema intern `supabase_functions` (el que fan servir els Database Webhooks de Supabase per fer la crida HTTP) no s'havia arribat a crear mai en aquest projecte, cosa que probablement també explicava per què les notificacions push tampoc funcionaven. Reproduït a mà (esquema + taula `hooks` + funció `http_request()`, mateixa estructura pública coneguda de Supabase) directament a la base de dades real, amb el propietari executant-ho ell mateix pel classificador de seguretat de l'entorn. **Pendent, com sempre**: provar el flux complet en viu (enviar, rebre el correu, acceptar amb compte nou i amb compte existent, revocar, reenviar).

* ~~Bienvenida de primer uso (onboarding) al abrir la app por primera vez.~~ **Fet.** No sorgeix de l'informe UX extern, sinó d'una petició directa del propietari: el primer ús podia resultar xocant sense cap explicació de què és Instriq ni què es pot fer sense compte. Nova `lib/screens/onboarding_screen.dart`: 3 passos a pantalla completa (què és Instriq / què pots fer sense compte -- catàleg, flashcards, progrés / com unir-te al teu equip quan vulguis), amb "Saltar"/"Següent"/"Comença". Es mostra **una sola vegada**: flag local `has_seen_onboarding` comprovat a `main.dart` en un `postFrameCallback`, mateix patró exacte que el ja existent avís de "nova versió disponible" (`SharedPreferences` inline, sense servei dedicat, `_navigatorKey.currentState?.push(...)`) -- deliberadament sense crear un `OnboardingService` nou quan el patró ja establert n'hi ha prou. No depèn de sessió ni connexió (coherent amb que el catàleg ja funciona en mode convidat): es comprova sempre, abans fins i tot que l'avís de versió. `flutter analyze`/`flutter test` nets, i verificat de veritat llançant `flutter run -d chrome` en aquesta mateixa sessió (primer llançament en aquest perfil de Chrome, sense el flag encara) — arrenca net, sense excepcions.

  **Ampliació**: selector d'idioma (català/castellà/anglès) també a la pròpia bienvenida — mateix diàleg exacte que ja existia a `ProfileHubScreen` (icona "Idioma"), extret a `lib/widgets/language_picker.dart` (`pickLanguage(context)`) perquè els dos llocs comparteixin el mateix codi en comptes de duplicar les 3 opcions. Persisteix igual que sempre via `LocaleService`/`SharedPreferences` (l'usuari ho descrivia com "guardar en les cookies" — és l'equivalent local ja establert a la resta de l'app, no cookies de navegador literals). `flutter analyze`/`flutter test` nets.
...
**Aquest paràgraf s'havia quedat sense actualitzar des de l'agost**: `knowledge_links` es va ampliar dues vegades més sense reflectir-ho aquí — `schema_v33_public_knowledge_links_and_profile.sql` (EPIC 9) hi va afegir `public_document`/`public_tray` a `from_type`, i `schema_v38_epic2_expansion.sql` (EPIC 2, sutures) hi va afegir `suture` a `to_type`. Corregit en revisar l'estat real del graf per continuar aquest EPIC (2026-09-15).

**Bug real trobat i corregit en aquesta revisió**: les 4 pantalles de "Usat a" (instrument de catàleg, instrumental personalitzat, safata, sutura) tenien el bucle de lectura de `knowledge_links` escrit ABANS que existís `public_document`/`public_tray` com a `from_type` possible — així que un enllaç sincronitzat des d'una tècnica o safata de la Biblioteca Pública es descartava silenciosament (el `for (final link in links)` no coincidia amb cap branca `if`), tot i que la fila ja existia a la base de dades des de fa mesos. No era una funcionalitat pendent, era una dada real invisible a la UI. Corregit a `instrument_detail_screen.dart`/`custom_instrument_detail_screen.dart` (les úniques dues afectades de veritat: `to_type` d'instrument és l'únic que travessa la frontera públic/privat -- una safata privada només pot ser referenciada per una tècnica privada, mai per contingut públic, i les sutures encara no tenen camí de sincronització des de contingut públic en absolut, `schema_v38` només va tocar l'RPC privada -- verificat directament al codi abans de tocar `tray_detail_screen.dart`/`suture_detail_screen.dart`, que es deixen tal qual perquè no tenen aquest bug). Ara naveguen a `PublicEntityDetailScreen.document()`/`.tray()` amb una etiqueta "Biblioteca Pública" per distingir-los visualment de contingut propi de l'organització.

**Forats reals confirmats, NO construïts (decisió conscient d'aquesta revisió, no una omissió)**: `preference_card ↔ tray` i `surgeon ↔ tray` ("bandeja preferida pel Dr. Garcia", l'últim salt de l'exemple aspiracional d'aquest EPIC) no existeixen enlloc del model de dades i no s'han construït ara -- cap dels dos té un cas d'ús concret que ho justifiqui (una targeta de preferència i una safata només comparteixen espai, mateix criteri ja documentat per a targeta↔tècnica; "bandeja preferida" no és com funciona el domini real, la targeta de preferència ja és el mecanisme per a preferències de cirurgià). `tècnica ↔ fabricant` roman indirecta a propòsit (via l'instrument relacionat, ja resolta a la UI d'EPIC 2 sense necessitat d'una fila de graf pròpia). Els tags (`taggings`) confirmats com a infraestructura genuïnament separada, no candidata a fusionar-se amb `knowledge_links`.

`flutter analyze`/`flutter test` nets.
...
**Estat: fet.** Aquest paràgraf s'havia quedat com "implementable ja" molt després de fer-se de veritat — mateix tipus de desactualització trobada i corregida abans a EPIC 9 aquesta sessió. `SyncQueueService` ja té la cua de 3 operacions (crear/desar esborrany/enviar a revisió) generalitzada a `Tray`/`PreferenceCard`/`SterilizationMethod`/`InstrumentTechnicalInfo` (13 `SyncOperationType`, no només tècniques), amb el mateix criteri exacte de l'ADR-003 (obrir capçalera nova o aprovar/rebutjar/restaurar exigeix connexió; continuar un esborrany ja obert s'encua). Els 4 models tenen `pendingSync` i les pantalles corresponents ja mostren `PendingSyncChip`. La UI de conflictes (`SyncFailure`) que l'ADR marcava com "falta construir" també existeix (`sync_issues_screen.dart`, accessible des de Perfil), contra el que deia aquest mateix paràgraf.

**Dos forats reals trobats i tancats en revisar-ho (2026-09-15)**, no tot estava fet:
1. **Lectura sense connexió d'esterilització/fitxa tècnica**: `OfflineCacheService` només cobria tècniques/bandejes/targetes (documentat expressament com "queda per a una ronda futura"). Afegit `toCacheRow()` a `SterilizationMethodEntry`/`SterilizationMethodVersion`/`InstrumentTechnicalInfo`/`InstrumentTechnicalInfoVersion` i 4 mètodes nous a `OfflineCacheService` (`cache`/`getCached` per a mètodes i fitxa tècnica, indexats per `refType`/`refId` d'instrument, no per workspace — no hi ha pantalla de llistat navegable, es consulta sempre des de la fitxa d'un instrument concret). `SterilizationService.fetchMethods`/`fetchTechnicalInfo` ara serveixen l'última caché coneguda sense connexió (o si la petició falla per xarxa), mateix criteri exacte que `TrayService.fetchTrays`. Cas especial gestionat: la fitxa tècnica és opcional per instrument, així que `null` és un valor cachejat vàlid ("confirmat sense fitxa"), distint de "mai s'ha cachejat" — calia distingir-los explícitament en deserialitzar.
2. **Avís "desat sense connexió" inconsistent entre pantalles**: `preference_card_form_screen.dart` i `clinical_data_form_sheet.dart` (esterilització/fitxa tècnica) no capturaven `wasOffline` ni mostraven `savedOfflineSnackbar` al desar, a diferència dels altres 3 formularis. Afegit el mateix patró exacte (`wasOffline` capturat abans de la crida, `SnackBar` + 900ms d'espera abans de tancar la pantalla).

**Límit conegut, no tocat aquesta ronda**: a la targeta de preferència, resoldre/crear un cirurgià nou (`SurgeonService.createOrGet`) encara exigeix connexió — si falla per xarxa, tomba tot el guardat abans d'arribar a la part que sí sap encolar-se. És una limitació pre-existent, no introduïda ara; arreglar-la seria fer `SurgeonService` offline-aware, fora d'abast d'aquesta correcció puntual.

`flutter analyze`/`flutter test` nets. **Pendent, com sempre**: provar en viu (compte autenticat, sense connexió real) la lectura cachejada d'esterilització i el guardat offline de targeta/esterilització.
...
Fora d'abast d'aquest tram: adopció d'una organització sobre contingut públic (Source/Parent/Sync Status d'ADR-001 -- encara no aplicat, mida d'EPIC sencera). `flutter analyze`/`flutter test` nets. Verificat en emulador en mode convidat: la Biblioteca Pública s'obre sense sessió, mostra les 2 pestanyes buides (encara no hi ha contingut publicat) sense cap excepció a `logcat`. El flux de proposta/revisió/aprovació complet no s'ha pogut provar en viu — requereix un compte de col·laborador real, mateix punt pendent que la resta de fluxos d'aquesta sessió.

**Estat: tercer tram fet (2026-09-15) — adopció d'organització sobre contingut públic, nomes bandejas.** Implementa ADR-001 §0 Opció C (referència upstream + divergència) sobre `supabase/schema_v42_tray_adoption.sql`, limitat deliberadament a bandejas -- el propi ADR §8 les assenyala com "el primer candidat real" (ja tenen versionat i duplicació, adoptar-ne una de pública és una extensió natural). Tècniques/targetes de preferència queden per a una ronda futura, mateix criteri de no generalitzar amb un únic consumidor ja aplicat a ADR-004.

Model de dades: 3 columnes noves a `trays` (`upstream_public_tray_id`, `upstream_adopted_version_id`, `sync_status` enum synced/customized/independent -- ADR-001 §0.7, la versió *decidida* pel propietari, que millora el booleà que la mateixa anàlisi de l'ADR proposava inicialment a §5) + un `check` que lliga els dos primers camps amb el tercer (o tots tres nuls, o cap). **Decisió d'interpretació explícita, documentada al propi fitxer SQL**: §2.3 del document parla d'esborrar la referència upstream en "Deixar de seguir", però §0 defineix `independent` com un valor del mateix enum -- es tria conservar sempre `upstream_public_tray_id` (procedència útil encara que ja no se segueixi) en comptes de perdre-la, fent servir `sync_status='independent'` per al mateix efecte pràctic (deixar de rebre avisos).

La transició `synced` → `customized` és automàtica (trigger `mark_tray_customized_on_content_change` sobre `tray_versions`, comparant només columnes de contingut -- mai status/timestamps -- perquè publicar la primera versió adoptada sense cap canvi no la marqui com a personalitzada per error), exactament com descriu ADR-001 §0.7 -- mai una acció manual. 3 RPC noves: `adopt_public_tray` (calcada de `duplicate_tray`, però la font és `public_trays`), `stop_following_public_tray` ("Deixar de seguir"), `update_tray_from_upstream` ("Actualitzar" -- porta el contingut ACTUAL de l'origen al propi esborrany en curs, mai publica sola, sempre cal "Enviar a revisió" com qualsevol altre canvi -- ADR-001 §9: mai una actualització silenciosa, ni tan sols en estat sincronitzat -- i desfà la marca `customized` que el seu propi trigger acaba de posar, perquè re-sincronitzar amb l'origen no és divergir-ne).

Codi Dart: `TraySyncStatus` (enum + `lib/models/tray.dart`), 3 mètodes nous a `TrayService`. UI: botó "Adopta com a bandeja pròpia" a `PublicEntityDetailScreen.tray()` (selector d'espai si n'hi ha més d'un, mateix criteri que `_openWorkspaceCollection` de `home_screen.dart`) que obre `TrayFormScreen` amb el nou esborrany per revisar abans d'enviar a revisió; `TrayDetailScreen` mostra una targeta amb l'estat (🟢/🟡/⚪, mai vocabulari "fork"/"branch"/"merge" -- ADR-001 §4) i, quan està sincronitzada però l'origen ja té una versió més nova (comprovat en cada càrrega comparant `upstream_adopted_version_id` contra el `published_version_id` actual de l'origen), un avís amb "Actualitza"/"Deixa de seguir".

Fora d'abast d'aquesta ronda, tal com recomanava el propi ADR: comparador estructurat camp a camp per a "Revisar canvis" en bandejas `customized` (§5, es tracta com un simple no-encara-suportat: el botó "Actualitza" només apareix en `synced`); notificació push quan l'origen publica (es comprova en llegir la fitxa, no en temps real -- evita necessitar un altre Database Webhook); tècniques/protocols i targetes de preferència.

`flutter analyze`/`flutter test` nets. **Pendent, com sempre**: aplicar la migració al projecte Supabase real i provar en viu tot el cicle (adoptar, editar per veure la transició a `customized`, publicar l'origen una versió nova i veure l'avís, actualitzar, deixar de seguir).

**Estat: tercer tram fet (2026-08-12, commit `3ee0824`) — perfil públic de col·laborador + sincronització de `knowledge_links`.** Aquest tram tancava 2 dels 3 forats que el paràgraf anterior deixava documentats (el tercer, adopció d'organitzacions sobre contingut públic, es confirma fora d'abast per ser mida d'EPIC sencera, no d'aquest tram). Implementat sobre `supabase/schema_v33_public_knowledge_links_and_profile.sql`: `knowledge_links` amplia `from_type`/`to_type` amb `public_document`/`public_tray`, i `approve_public_document_version`/`approve_public_tray_version` sincronitzen igual que les seves bessones privades (mateix bloc, `organization_id` sempre null); neteja en esborrar contingut públic afegida a `cleanup_knowledge_links()`. Nova `ContributorPublicProfileScreen` (lectura, oberta a convidats): nom públic, nivell, organització (només si l'ha triat mostrar — `get_public_contributor_organization`, security definer, comprova el consentiment abans de saltar-se la RLS de `profiles`/`organizations` entre organitzacions), àrees de col·laboració, contribucions publicades — accessible tocant l'autor a `PublicEntityDetailScreen`. `flutter analyze`/`flutter test` nets.

**Estat: quart tram fet (2026-09-25) — wizard de creació de grup/espai.** Sorgeix d'una conversa sobre com fer l'app més atractiva per a hospitals/universitats/equips petits: personalitzar la primera experiència en comptes de començar amb una pantalla en blanc. `supabase/schema_v49_workspace_wizard.sql`: (1) `register_hospital` guanya `p_org_type` (`organizations.org_type` ja existia des de `schema_v20_organizations_rename.sql` amb les 6 categories de la landing "Per a qui", però mai s'escrivia des del client — tot grup creat fins ara ha quedat `'hospital'` per defecte encara que no ho fos; calia `drop function` abans de `create`, una signatura amb un paràmetre nou no reemplaça l'existent, en crea una de sobrecarregada); (2) `workspaces.specialty_id`, nou; (3) `starter_sets` (especialitat -> safates públiques recomanades), curat pel Consell Editorial, mateixa RLS lectura-oberta/escriptura-reviewer-or-above que la resta de la Biblioteca Pública. Deliberadament NO hi ha cap RPC nova d'adopció en bloc: el client crida `adopt_public_tray` un cop per fila seleccionada (`TrayService.adoptPublicTray`, ja existia), evitant duplicar lògica d'adopció en SQL.

Codi Dart: `OrgType` (enum + extensió `.label(l10n)`, mateix criteri que `InstrumentCategoryLabel` arreglat aquesta mateixa sessió — mai un getter amb text fix), selector de 6 targetes a `GroupEntryScreen._buildCreateForm`; selector d'especialitat opcional al diàleg de `_createWorkspace()` (`workspace_list_screen.dart`); nova `WorkspaceStarterPackScreen` (checkboxes preseleccionats, 100% saltable) si `starter_sets` té contingut curat per a l'especialitat triada — **avui la Biblioteca Pública no té cap safata publicada encara, així que aquest darrer pas sempre queda ocult fins que algú (Consell Editorial) curi `starter_sets` amb contingut real; la infraestructura ja hi és, poblar-la és una tasca operativa posterior, no d'aquest tram**.

Revisió adversarial multi-dimensió (seguretat/RLS, correctesa, consistència) feta abans d'aplicar a producció: seguretat sense troballes; 3 bugs reals trobats i corregits abans de pujar — el bucle d'adopció de `WorkspaceStarterPackScreen` no portava traça de què ja s'havia adoptat (un fallo a mitja llista + reintent duplicava safates, `adopt_public_tray` no és idempotent — ara es marca cada safata adoptada i es salta en un reintent, amb missatge que diu quantes ja s'han afegit); `WorkspaceService.renameWorkspace()` perdia `specialtyId` en reconstruir el `Workspace` cachejat (no l'incloïa al constructor, a diferència de `fromRow`); la nova crida a `SpecialtyService.fetchAll()` dins `_createWorkspace()` no tenia `try/catch`, a diferència de tots els altres llocs on es crida (formularis de contingut), cosa que hauria trencat el diàleg sencer si fallava.

Aplicat a producció i verificat en viu: funció de 3 paràmetres present, cap sobrecàrrega de 2 vella, cap grant a `anon`, `workspaces.specialty_id` i `starter_sets` presents. `flutter analyze`/`flutter test` nets. **Pendent, com sempre**: provar el flux complet en viu amb compte autenticat (crear grup triant tipus, crear espai triant especialitat), i — quan hi hagi contingut real a la Biblioteca Pública — curar `starter_sets` per a almenys una especialitat i provar el pas del starter pack, que avui no s'ha pogut exercitar de cap a cap per manca de dades.

**Nota (2026-09-14)**: aquest paràgraf s'havia quedat sense actualitzar des del commit `3ee0824` — el `BACKLOG.md` encara llistava tots dos punts com a "fora d'abast" mesos després de fer-se; `docs/EPIC_COMMUNITY_GOVERNANCE.md` sí que es va actualitzar en el seu moment. Corregit en revisar l'estat del projecte per continuar-ne el desenvolupament.
